// Private fork of prototype/look.mjs for J1a basemap-candidate judging ONLY — lives outside
// prototype/ on purpose (the brief forbids editing look.mjs itself). Same shader arithmetic,
// plus a base= arg so different basemap candidates can be pushed through the real render.
// Not shipped, not kept in lockstep beyond this task.
import fs from 'fs';
...
const dataPath = f => path.join(PROTOTYPE, 'data', f);
...
const BASE = A.base ?? 'base-aerial.jpg'; // filename inside prototype/data/
...
const baseObj = await sharp(dataPath(BASE)).removeAlpha().toColourspace('srgb').raw().toBuffer({resolveWithObject:true});
const BP = baseObj.info.width, base = baseObj.data;
const fldObj = await sharp(dataPath('field-v2.png')).removeAlpha().toColourspace('srgb').raw().toBuffer({resolveWithObject:true});
