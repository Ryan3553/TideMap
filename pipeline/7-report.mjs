...
let CLEAN = null; try { CLEAN = R('clean.json'); } catch { /* stage 9 not run */ }
...
${CLEAN ? `**Read the rasters as stage 9 leaves them.** \`9-clean.mjs\` rewrites \`classes.png\` and
...
**${CLEAN.afterHarbourIntertidalKm2 ?? '133.3'} km²**. If you re-run \`4-fit.mjs\` you must re-run
...
${CLEAN ? `## 7. Stage 9 — water that is not water
...
**Choosing the cut.** Test 2's threshold is \`WET_MIN\`, set to **${CLEAN.wetMinNdwi}** NDWI. The
...
| | before | after |
|---|---|---|
| subtidal | ${CLEAN.beforeKm2.subtidal} km² | ${CLEAN.afterKm2.subtidal} km² |
| intertidal (whole frame) | ${CLEAN.beforeKm2.intertidal} km² | ${CLEAN.afterKm2.intertidal} km² |
| **intertidal inside \`harbour-mask.png\`** | **${CLEAN.beforeHarbourIntertidalKm2} km²** | **${CLEAN.afterHarbourIntertidalKm2} km²** |

Removed: ${CLEAN.removedKm2.spectralIntertidal} km² intertidal by the spectral test,
${CLEAN.removedKm2.orphanIntertidal} km² intertidal + ${CLEAN.removedKm2.orphanSubtidal} km²
subtidal as orphan components (${CLEAN.orphanComponents} of them).

**The cost is ${(CLEAN.beforeHarbourIntertidalKm2 - CLEAN.afterHarbourIntertidalKm2).toFixed(1)} km² of harbour intertidal, ${(100 * (CLEAN.beforeHarbourIntertidalKm2 - CLEAN.afterHarbourIntertidalKm2) / CLEAN.beforeHarbourIntertidalKm2).toFixed(1)}% of the scored set,
